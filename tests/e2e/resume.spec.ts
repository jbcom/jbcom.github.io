...
test.describe('CV page (Astrofy timeline)', () => {
  test('cv page renders profile, experience, skills, and education', async ({ page }) => {
    ...
    await expect(page.locator('#profile-summary').getByText('Profile')).toBeVisible()
    await expect(page.locator('#experience').getByText('Experience')).toBeVisible()
    await expect(page.locator('#skills').getByText('Skills')).toBeVisible()
